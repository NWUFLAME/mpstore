...
mpstore.Page(store, {
  ...
  handleChooseImg() {
    ...
    wx.chooseImage({
      ...
      success: (result) => {
        this.commit('updateChooseImgs', [...this.data.chooseImgs, ...result.tempFilePaths])
      }
    });
  ...
  handleRemoveImg(e) {
    ...
    this.commit('deleteChooseImg', {index})
  },
  // 文本域的输入的事件
  handleTextInput(e) {
    this.commit('updateTextVal', e.detail.value)
  },
  // 提交按钮的点击
  handleFormSubmit() {
    // 1 获取文本域的内容 图片数组
        ...
        wx.uploadFile({
          ...
          success: (result) => {
            ...
            if (i === chooseImgs.length - 1) {
              ...
              //  提交都成功了
              // 重置页面
              this.commit('updateTextVal', '')
              this.commit('updateChooseImgs', [])
              // 返回上一个页面
              wx.navigateBack({
                delta: 1
              });
